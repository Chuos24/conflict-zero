...
DECOLECTA_API_KEY = os.environ.get('DECOLECTA_API_KEY', 'sk_13991.akU3WqHys0VVBtheNHbbiAeVUlW24BNp')
...
def call_decolecta_api(endpoint):
    ...
    try:
        ...
        print(f"Error llamando Decolecta API {endpoint}: {e}")
        return None

def get_sunat_data_real(ruc):
    """Obtiene datos reales de SUNAT via Decolecta"""
    try:
        # Endpoint correcto según documentación Decolecta
        result = call_decolecta_api(f'v1/sunat/ruc/{ruc}')
        
        print(f"Decolecta SUNAT response: {json.dumps(result, default=str)[:500]}")
        
        if result:
            # Decolecta retorna directamente los datos o en formato específico
            if isinstance(result, dict):
                ...
                return {
                    'razon_social': data.get('razon_social') or data.get('nombre') or data.get('razonSocial', ''),
                    'nombre_comercial': data.get('nombre_comercial') or data.get('nombreComercial', ''),
                    ...
                    'tipo': data.get('tipo') or data.get('tipoContribuyente', 'EMPRESA'),
...
def get_reniec_data_real(dni):
    """Obtiene datos de RENIEC via Decolecta (para representantes)"""
    try:
        result = call_decolecta_api(f'v1/reniec/dni/{dni}')
        if result and isinstance(result, dict):
            return {
                'nombres': result.get('nombres', ''),
                'apellido_paterno': result.get('apellido_paterno') or result.get('apellidoPaterno', ''),
                'apellido_materno': result.get('apellido_materno') or result.get('apellidoMaterno', ''),
                'fuente': 'decolecta_reniec'
            }
    except Exception as e:
        print(f"Error RENIEC data: {e}")
    return None

def get_mock_osce_data(ruc, razon_social):
    """Genera datos mock deterministas para OSCE/TCE basado en RUC"""
...
def get_ruc_data(ruc):
    ...
    # Si falla, usar fallback con determinístico
    if not sunat_data:
        seed = generate_ruc_seed(ruc)
        mock_names = [
            ...
            "CONSTRUCTORA ZAMORA JARA SAC", "GRUAS Y MONTACARGAS PERU SAC"
        ]
        ...
        sunat_data = {
            ...
            'estado': 'ACTIVO',
            'condicion': 'HABIDO',
            'direccion': f"Av. {['Principal', 'Lima', 'Arequipa', 'Trujillo', 'Cuzco'][pseudo_random(seed+1, 5)]} N° {pseudo_random(seed+2, 9999)}",
            'departamento': ['Lima', 'Arequipa', 'La Libertad', 'Cusco', 'Junin'][pseudo_random(seed+3, 5)],
            'provincia': ['Lima', 'Arequipa', 'Trujillo', 'Cusco', 'Huancayo'][pseudo_random(seed+4, 5)],
            'distrito': '',
            'tipo': 'EMPRESA',
            'telefono': '',
    ...
    return {
        ...
        'telefono': sunat_data.get('telefono', ''),
...
def upload_certificate_to_s3(ruc, html_content):
    """Sube certificado a S3 y retorna URL pública"""
    try:
...
def lambda_handler(event, context):
    ...
    try:
        ...
        # Obtener datos (SUNAT real + OSCE/TCE mock)
            ...
            if not pdf_url:
                return {
                    ...
                    'body': json.dumps({
                        ...
                        'message': 'Error al subir a S3. Verifique permisos.',
                        'fuentes_datos': data.get('fuentes_datos', {})
                    })
                }
